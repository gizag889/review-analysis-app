...
export default function useAISummaryLogic() {
  ...
  const prompt = `Write a summary of the reviews for the tech event.
      ...
      IMPORTANT: Do not wrap the JSON response in markdown code blocks (like \`\`\`json and \`\`\`). 
      Return only the raw JSON object without any formatting or code block markers.
